...
impl VM {
  ...
  pub fn run(&mut self) -> RuntimeResult {
    loop {
      let exec = match self.next_byte().into() {
        ...
        OpCode::Multiply => self.op_binary_expression(BinaryExprKind::Mult),
  ...
  fn op_load_imm_false(&mut self) -> OpRes {
    ControlFlow::Continue(self.stack.push(false.into()))
  }
  ...
  fn op_load_imm_true(&mut self) -> OpRes {
    ControlFlow::Continue(self.stack.push(true.into()))
  }

  #[inline(always)]
  fn op_load_native_or_long(&mut self, is_long: bool) -> OpRes {
    let native_idx = operand![self, is_long] as usize;
    ControlFlow::Continue(self.stack.push(Object::NativeFunc(native_idx)))
  }
  ...
  fn op_binary_expression(&mut self, operator: BinaryExprKind) -> OpRes {
    ...
    let result = match operator {
      ...
      BinaryExprKind::Mult => val1.mul(val2, &mut self.gc),
